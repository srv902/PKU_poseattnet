...
# import sys
# sys.path.append(os.path.abspath(os.path.join('..', 'i3d')))
...
from i3d_train import i3d_modified
...
def GCNN_skeleton_t10(num_nodes, num_features, graph_conv_filters_shape1, graph_conv_filters_shape2, num_filters, num_classes, n_neuron, n_dropout, timesteps):
    print('Build GCNN')
    X_input_t1 = Input(shape=(num_nodes, num_features))
    X_input_t2 = Input(shape=(num_nodes, num_features))
    X_input_t3 = Input(shape=(num_nodes, num_features))
    ...
    output = keras.layers.Concatenate(axis=1)([output1, output2, output3, output4, output5, output6, output7, output8, output9, output10])
    output = keras.layers.Concatenate()([output, X_input])
    out = BatchNormalization()(output)
    out = Conv2D(64, (3, 3), activation='relu', padding='same')(out)
    #out = MaxPooling2D(pool_size=(2,2))(out)
    ...
    #output_final = Dense(num_classes, activation='softmax')(out_new)
    model = Model(inputs=[X_input_t1, X_input_t2, X_input_t3, X_input_t4, X_input_t5, X_input_t6, X_input_t7,
                          X_input_t8, X_input_t9, X_input_t10, X_input, graph_conv_filters_input], outputs=out_new)
    return model
...
def embed_model_spatio_temporal_gcnn(n_neuron, timesteps, num_nodes, num_features,
    ...
    # load pretrained i3d on 16 frames PKU data ..
    if num_classes == 52:
        pass  # model_branch.load_weights("/data/stars/user/sasharma/PKU_poseattnet/output/pku_rgb_cross_subject_16I3D/weights_pku_rgb_cross_subject_16I3D/epoch9.hdf5")
    else:
        pass
    ...
    model_gcnn = GCNN_skeleton_t10(num_nodes, num_features, graph_conv_filters_shape1,
                                   graph_conv_filters_shape2, num_filters, num_classes,
                                   n_neuron, n_dropout, timesteps)
